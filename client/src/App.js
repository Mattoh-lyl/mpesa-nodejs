...
setserver('! Enter a valid amount')
     ...
     setserver("")
   await axios.post(' http:localhost:444/stk/push', {pno,amount}).then((response)=>{ /*//http:localhost:444/stk..../ https://trympesa.herokuapp.com/stk/push */
    ...
    if(response.data.ResponseDescription==="Success. Request accepted for processing")
     {
      ...
      swal({
        title: "Success!",
        text: "Payment information sent to +"+ pno +" kindly complete transaction on your phone",
        icon: "success",
        button: "Close",
      }).then((okey)=>{
        if(okey)
